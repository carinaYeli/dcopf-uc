function [result]=dp(an)
%% initialization of parameters 
global mpc initial_state Time Max_feasible_states load gen_min gen_max cn
time = 24;
eps = 1*10^(-6);
...
%% firstly delete the infeasible states for each time
% determine feasible states in each step by figureing out which combination
% is larger than the total load in each step
Max_feasible_states = length(new_combination_capacity)-find(new_combination_capacity>min(total_load),1)+1;% max number of states that could statisfy the load
...
%% initiallization of parameter matrix for dynamic programming
...
%% dynamic programming plus dcopf for time period
while Time<=time 
    ...
    k_next_state(1:feasible_states_number(Time),gen_num+1)=[feasible_Index_stage(Time):(feasible_Index_stage(Time)+feasible_states_number(Time)-1)]';
        ...
        infeasible=0;
        % using dcopf to shrink range of feasible states in each time 
        for kns=1:k_next_state_row % determine feasible states and their power output, production cost in each time using dcopf, based on the previpus feasible_combination result 
            ...
            % determine which algoritim ashould be used
            if an==1 % do matpower 
                [cost,f_state,ge_out,power_flow,theta]=matpower_dcopf(feasible_k_next_state);
            elseif an==2 % do quaduatic programming
                [cost,f_state,ge_out,power_flow,theta]=quad_dcopf(feasible_k_next_state,Time,load);
            end 
                               
            if f_state==0 %set the cost of infeasible states to zero for the convenient of next rearrangement of parameter matrix
 ...
 %% dynamicly calculate the transmission cost and total cost, decide which states should be coneected with previous states

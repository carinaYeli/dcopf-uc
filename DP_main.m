...
global mpc initial_state Time Max_feasible_states
time=24;
eps=1*10^(-6);
mpc=loadcase('case14');
...
load14=mpc.bus(:,3)*1.8;
load15=mpc.bus(:,3)*1.9;
load16=mpc.bus(:,3)*2;
load17=mpc.bus(:,3)*1.7;
load18=mpc.bus(:,3)*2.2;
...
%             [cost,f_state,answer,power_flow,theta]=quad_dcopf(feasible_k_next_state,Time,load);%generate production cost for each stage
            [cost,f_state,answer,power_flow,theta]=dcopf(feasible_k_next_state,Time,load);
            ...
            generator_output=answer(1:gen_num)';% store each state's generation output
...
%      if m==1
%         [min_cost,find_state]=min(total_cost_combination(m,1:sum(any(total_cost_combination(m,:),1))));
% %         from_state=2;
%         to_state=path_record(find_state,m+1);
%         path(1,m)=to_state;
% %         find_state=1;
%      else  
%         find_state=find(~(path_record(:,2*m-1)-to_state));
%         [min_cost,from_state]=min(total_cost_combination(m,1:sum(any(total_cost_combination(m,:),1))));
% %         find_state=find(~(path_record(:,2*m)-to_state));
% %         to_state=path_record(find_state,2*m-1);
% %         from_state=path_record(find_state,2*m-1);
%         to_state=path_record(from_state,2*m);
%         path(1,m)=to_state;
%      end
%         result(m,1:gen_num)=new_combination(path(m),:);
%         Index_state=find(feasible_combination(m,:)==path(m));
%         result(m,(gen_num+1):2*gen_num)=power_combination(m,((Index_state-1)*gen_num+1):Index_state*gen_num);
%         result(m,2*gen_num+1:2*gen_num+branch_num)=power_flow_combination(m,((Index_state-1)*branch_num+1):Index_state*branch_num);
%         result(m,2*gen_num+branch_num+1)=sum(power_combination(m,((Index_state-1)*gen_num+1):Index_state*gen_num));
%         result(m,2*gen_num+branch_num+2)=total_cost_combination(m,Index_state);
% end

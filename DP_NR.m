function [result]=DP_NR
global mpc initial_state T Max_feasible_states load gen_min gen_max
...
result = zeros(time,2*gen_num+branch_num+bus_number+2);
Time = 1;
T = 1;
exitflag = 0;
while Time<=time 
    ...
    k_next_state(1:feasible_states_number(Time),gen_num+1)=[feasible_Index_stage(Time):(feasible_Index_stage(Time)+feasible_states_number(Time)-1)]';
        ...
        for kns=1:k_next_state_row % determine feasible states and their power output, production cost in each time using dcopf, based on the previpus feasible_combination result 
            ...
            [cost,f_state,answer,power_flow,theta]=NR_dcopf(feasible_k_next_state); %generate production cost for each stage
        ...
        if sum(production_cost_combination(Time,:))==0
            exitflag=1;
            break
        end
...
% restore the combination of different biding constraints 
